...
#         )
    ...
    def get_vec(photo_id):
        return resnet.get_vec(Image.open(f'photos/{photo_id}.jpg'))
...
#         business_history_ratings = eval(data.sequence_ratings)
#         photo_history = eval(data.sequence_photo_ids)
...
#         target_photo = torch.unsqueeze(torch.from_numpy(YelpDataset.get_vec(photo_history[-1:][0])), 0)
...
#         photo_history = torch.stack([torch.from_numpy(YelpDataset.get_vec(photo)) for photo in photo_history][:-1])
...
#         return user_id, business_history, target_business_id,  business_history_ratings, target_business_rating